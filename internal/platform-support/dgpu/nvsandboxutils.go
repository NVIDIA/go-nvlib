...
import (
	...
	"github.com/NVIDIA/go-nvlib/internal/nvsandboxutils"
)
...
func (d *nvsandboxutilsDGPU) Devices() ([]discover.Device, error) {
	...
	for _, info := range gpuFileInfos {
		switch info.SubType {
		case nvsandboxutils.NV_DEV_DRI_CARD, nvsandboxutils.NV_DEV_DRI_RENDERD: //nolint:misspell // Upstream constant name uses RENDERD.
			if d.isMig {
				continue
			}
			fallthrough
		...
		case nvsandboxutils.NV_DEV_DRI_CARD_SYMLINK, nvsandboxutils.NV_DEV_DRI_RENDERD_SYMLINK: //nolint:misspell // Upstream constant name uses RENDERD.
			if d.isMig {
				continue
			}
