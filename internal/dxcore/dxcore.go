...
func initContext() (*context, error) {
	...
	if C.dxcore_init_context(&cContext) != 0 { //nolint:gocritic // cgo call expands in a way that triggers dupSubExpr.
...
func (c context) deinitContext() {
	...
	C.dxcore_deinit_context(&cContext) //nolint:gocritic // cgo call expands in a way that triggers dupSubExpr.
}
